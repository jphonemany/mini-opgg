...
Users can search a Riot ID, such as `GameName#TagLine`, and view player profile data, ranked solo queue stats, recent match history, and simple performance analytics.
...
```bash
docker run --name mini-opgg-postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=mini_opgg -p 5432:5432 -d postgres:16
```
...
- `RIOT_REGIONAL_ROUTE`: used by Account-V1 and Match-V5, for example `americas`, `asia`, `europe`.
- `RIOT_PLATFORM_ROUTE`: used by Summoner-V4 and League-V4, for example `na1`, `euw1`, `kr`.
...
Mini OP.GG is a full-stack League of Legends analytics dashboard that demonstrates external API integration, backend service layering, DTO-based API responses, PostgreSQL persistence, cache freshness checks, and a responsive React UI.
